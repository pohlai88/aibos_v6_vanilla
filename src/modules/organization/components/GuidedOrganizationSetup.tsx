...
import { useGuidance, WelcomeBanner, ContextualTooltip } from '../../../components/OnboardingGuidance';

interface GuidedOrganizationSetupProps {
  userId: string;
  userEmail?: string;
...
const GuidedOrganizationSetup: React.FC<GuidedOrganizationSetupProps> = ({
  userId,
  userEmail,
  onComplete,
  onSkip
}) => {
  const [currentStep, setCurrentStep] = useState<'welcome' | 'basic' | 'advanced' | 'team' | 'complete'>('welcome');
  ...
  useEffect(() => {
    ...
    if (!isPopupDismissed('welcome-popup')) {
      showPopup('welcome-popup', 'Welcome to AIBOS! Let\'s get your organization set up in just a few minutes.');
    }
  }, [userEmail, showPopup, isPopupDismissed]);
  ...
  // Show hotspots for important elements
  useEffect(() => {
    if (currentStep === 'basic') {
      // Show hotspot for organization name field after a short delay
      setTimeout(() => {
        showHotspot('#organization-name', 'Start here! Enter your organization name.');
      }, 1000);
    }
  }, [currentStep, showHotspot]);
  ...
  const handleBasicSubmit = async (data: Record<string, any>) => {
    ...
    hideHotspot('#organization-name'); // Hide hotspot when user starts typing
    ...
    try {
      ...
      // Show success popup
      showPopup('basic-complete', 'Great! Basic information saved. Let\'s continue with additional settings.');
  ...
  const handleAdvancedSubmit = async (data: Record<string, any>) => {
    ...
    try {
      ...
      // Show success popup
      showPopup('advanced-complete', 'Advanced settings configured! Your organization is now optimized for compliance and reporting.');
  ...
  const handleSkipAdvanced = async () => {
    ...
    showPopup('advanced-skipped', 'No problem! You can configure advanced settings later in your organization settings.');
  ...
  const handleSkipTeam = async () => {
    ...
    showPopup('team-skipped', 'Team invitations can be sent later. You\'ll find the option in your organization settings.');
    await createOrganization(organizationData);
  };
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
              currentStep === 'welcome' ? 'bg-gray-300 text-gray-500' :
              currentStep === 'basic' ? 'bg-blue-600 text-white' : 'bg-green-600 text-white'
            }`}>
              1
            </div>
            <span className={`text-sm font-medium ${
              currentStep === 'welcome' ? 'text-gray-500' :
          ...
          <div className="flex-1 mx-4 h-0.5 bg-gray-200">
            <div className={`h-full bg-blue-600 transition-all duration-300 ${
              currentStep === 'welcome' ? 'w-0' : 'w-full'
            }`}></div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className={`text-sm font-medium ${
              currentStep === 'basic' ? 'text-blue-600' : 
      ...
      <div className="md:hidden">
        <div className="flex items-center justify-between mb-4">
          <span className="text-sm font-medium text-gray-700">
            Step {currentStep === 'welcome' ? '1' : currentStep === 'basic' ? '2' : currentStep === 'advanced' ? '2' : currentStep === 'team' ? '3' : '4'} of 4
          </span>
    ...
    <div className="space-y-6">
      {showWelcomeBanner && (
        <WelcomeBanner
          ...
          description="Let's get your organization set up quickly and easily. We'll guide you through each step."
          checklist={[
            "Basic organization information",
            "Advanced settings (if needed)",
            "Team member invitations (optional)",
            "Ready to start using AIBOS"
          ]}
          onDismiss={() => setShowWelcomeBanner(false)}
          onStartTour={() => startTour('organization-setup')}
        />
      )}
      ...
      <div className="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-4">
        <ContextualTooltip content="Start the guided setup process with helpful tips along the way">
          <button
        ...
        <ContextualTooltip content="Skip the guided tour and go directly to the setup form">
          <button
  ...
  const renderBasicStep = () => (
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600 text-sm md:text-base">
          Let's start with the essential details about your organization.
        </p>
      </div>
...
export default GuidedOrganizationSetup; 